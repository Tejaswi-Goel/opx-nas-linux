    ...
    if (add && (nas_os_add_port_to_lag(name_obj) != STD_ERR_OK)) {
       ...
       /*  delete the dummy interface  */
        if (nas_os_delete_dummy((const char *)bond_dummy_name) != STD_ERR_OK) {
            EV_LOGGING(NAS_OS, ERR, "NAS-OS-LAG",
               "Error deleting  dummy port %d in the Kernel", ifix);
            return (STD_ERR(NAS_OS, FAIL, 0));
        }
    }
...
t_std_error nas_os_delete_lag(cps_api_object_t obj)
{
    ...
    /* Remove the VRF association if exists before deleting the LAG interface */
    if (nas_remove_intf_to_vrf_binding(if_index) != STD_ERR_OK) {
        EV_LOGGING(NAS_OS, ERR, "NAS-OS-LAG", "Error deleting VRF association from LAG:%s",
                   if_lag_name);
    }
