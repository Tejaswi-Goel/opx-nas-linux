...
t_std_error nas_os_create_bridge(cps_api_object_t obj, hal_ifindex_t *br_index)
{
     ...
     */
    unsigned int flags = IFF_BROADCAST | IFF_PROMISC | IFF_MULTICAST;
    auto admin_attr = cps_api_object_attr_get(obj, IF_INTERFACES_INTERFACE_ENABLED);
    if (admin_attr != nullptr && (bool)cps_api_object_attr_data_uint(admin_attr)) {
        flags |= IFF_UP;
    }
    nas_os_pack_if_hdr(ifmsg, AF_BRIDGE, flags, if_index);
    ...
    // Add MTU if it in object
    auto mtu_attr = cps_api_object_attr_get(obj, DELL_IF_IF_INTERFACES_INTERFACE_MTU);
    if (mtu_attr != nullptr) {
        int mtu = (int)cps_api_object_attr_data_uint(mtu_attr) - NAS_LINK_MTU_HDR_SIZE;
...
{
    return nas_os_create_bridge(obj, br_index);
}
